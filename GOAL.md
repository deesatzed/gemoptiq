...
- `pytest -q` from `/Volumes/WS4TB/gemOptq` passes with `136 passed`.
...
- `python scripts/readiness_check.py` and `sentinel readiness` run the local disposable proof matrix. Default local smoke mode reports `10 pass`, `5 manual`, and `1 external_blocked`. Optional real-agent smokes can be run explicitly; `python scripts/readiness_check.py --run-claude-trust-smoke --run-codex-exec-smoke` currently reports `12 pass`, `3 manual`, and `1 external_blocked` when Codex exec is allowed to initialize its local app-server client.
