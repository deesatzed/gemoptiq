...
- `pytest -q` from `/Volumes/WS4TB/gemOptq` passes with `142 passed`.
...
- `venv/bin/python scripts/auditor_smoke.py` passes when run with local Metal access and loads `mlx-community/gemma-4-12B-it-OptiQ-4bit`, producing a structured `allow` / `green` verdict with measured load and audit timing.
- `python scripts/readiness_check.py` and `sentinel readiness` run the local disposable proof matrix. Default local smoke mode reports `10 pass`, `6 manual`, and `1 external_blocked` because real-auditor loading and real-agent Codex TUI/exec checks are opt-in. Optional real-agent and real-auditor smokes can be run explicitly; `python scripts/readiness_check.py --run-real-auditor --run-claude-trust-smoke --run-codex-exec-smoke --run-codex-tui-smoke` currently reports `14 pass`, `3 manual`, and `0 external_blocked` when local Metal and Codex app-server access are available.
...
- `python scripts/real_agent_smoke.py --codex-exec-smoke --timeout 30` passes when run with local app-server access and confirms Codex can perform a harmless shell `command_execution` in an ephemeral temp workspace, producing `SENTINEL_CODEX_OK`.
- `python scripts/real_agent_smoke.py --codex-tui-smoke --timeout 60` passes and confirms Sentinel can drive Codex's full-screen TUI in an ephemeral git workspace through workspace trust, interactive command approval, and harmless command output `SENTINEL_CODEX_TUI_OK`.
...
- Default readiness intentionally leaves the real Gemma auditor smoke external-blocked unless `--run-real-auditor` is provided; restricted or non-Metal sessions can still fail with no Metal access.
- PTY-style interaction is proven against disposable synthetic agent fixtures, including a sequential startup-plus-tool-confirmation-shaped fixture. `PtyAgentRunner` now answers a cursor-position query fixture, reports a usable default PTY window size, and responds to a small set of terminal capability/color queries. Codex, Claude Code, and Gemini CLI executables are discoverable through safe non-interactive metadata probes. Claude Code startup trust-prompt control is proven in a disposable workspace. Codex non-interactive model/tool command execution and Codex full-screen TUI command approval are proven in ephemeral workspaces. Full interactive Claude/Gemini tool-confirmation behavior remains unproven; the latest Claude attempt trusted the temp workspace and typed the harmless prompt but did not reach a tool permission prompt before timeout, and Gemini prompted for browser authentication.
...
### GAP-001: Real Model Runtime Is Verified In Metal-Capable Sessions

**Severity:** Reduced from High to Low-Medium
**Current state:** The repo `venv` has the required `gemma4_unified -> gemma4` mapping, `scripts/auditor_smoke.py --dry-run` proves structured smoke reporting, and `venv/bin/python scripts/auditor_smoke.py` passes when run with local Metal access. The verified real smoke loads `mlx-community/gemma-4-12B-it-OptiQ-4bit`, returns a structured `allow` / `green` verdict, and reports measured load/audit timing. Default readiness still treats this as opt-in because restricted or non-Metal sessions may fail with no Metal device.
...
**Current state:** `PtyAgentRunner` now captures prompts without newline, injects input, prevents double-start, kills process groups, can launch commands in a specified disposable working directory, answers basic terminal queries such as cursor-position reporting, and starts children with a usable 24x80 PTY window size instead of the platform default 0x0 size. `scripts/agent_integration_smoke.py` proves prompt detection, input injection, disposable file write, and kill behavior in a temporary workspace. `scripts/real_agent_smoke.py` provides a guarded opt-in harness for caller-supplied real-agent commands, requiring an explicit command and running it in a temporary workspace. It supports repeated `--interaction "REGEX=>INPUT"` steps, rejects expected-output matches that appear before any injected input, and accepts expected output that arrives after a prior scripted input while a later optional prompt is still pending. It also has safe non-interactive CLI metadata probes; in this session, Codex, Claude Code, and Gemini version probes pass. `python scripts/real_agent_smoke.py --claude-trust-smoke --timeout 8` proves Sentinel can drive Claude Code's startup trust prompt in a disposable workspace, inject the safe `No, exit` response, and kill the process. `python scripts/real_agent_smoke.py --codex-exec-smoke --timeout 30` proves real Codex model/tool command execution in an ephemeral temp workspace when Codex can initialize its local app-server client. `python scripts/real_agent_smoke.py --codex-tui-smoke --timeout 60` proves Codex full-screen TUI workspace trust, interactive command approval, and harmless command output in an ephemeral git workspace. A real Claude Code tool-confirmation attempt using the multi-step harness trusted the temp workspace and typed the harmless prompt but did not reach a tool permission prompt or produce the encoded expected output before timeout. A Gemini attempt reached a browser authentication prompt outside the harness. Full interactive Claude/Gemini tool-confirmation behavior remains unproven until the harness is run successfully with deliberately safe commands that exercise those flows.
...
**Acceptance evidence:** `tests/test_pty_runner.py` passes, including cursor-position query response and usable PTY window-size fixtures; `venv/bin/python scripts/agent_integration_smoke.py` exits 0 with `prompt_detected`, `input_injected`, `file_written`, and `process_killed` all true. `tests/test_real_agent_smoke.py` proves the guarded harness dry-run, explicit-command requirement, disposable cwd execution, prompt detection, input injection, expected output detection, sequential multi-prompt interactions, process kill, non-interactive probe reporting, named Claude trust-prompt smoke wiring, named Codex exec smoke parsing, named Codex full-screen TUI smoke wiring, and post-input expected-output handling. `python scripts/real_agent_smoke.py --probe-installed-agents --timeout 5` exits 0 with `pass_count: 3` for Codex, Claude Code, and Gemini. `python scripts/real_agent_smoke.py --claude-trust-smoke --timeout 8` exits 0 with `prompt_detected`, `input_injected`, and `process_killed` true. `python scripts/real_agent_smoke.py --codex-exec-smoke --timeout 30` exits 0 with `tool_output_detected: true`, `tool_exit_code: 0`, and `proves_model_or_tool_behavior: true` when local app-server access is allowed. `python scripts/real_agent_smoke.py --codex-tui-smoke --timeout 60` exits 0 with two interactions detected/injected, `interactive_prompt_control: true`, `proves_model_or_tool_behavior: true`, and `SENTINEL_CODEX_TUI_OK` in `response_text`. Full interactive Claude/Gemini tool-confirmation commands still need to complete successfully through the PTY harness.
...
**Severity:** Reduced from High to Medium
**Current state:** `Auditor.audit_intent_result` now requires a structured JSON verdict and the legacy tuple API delegates to that parser. Free-text `YES` no longer approves. A real `scripts/auditor_smoke.py` run has verified one Metal-backed Gemma 4 structured verdict path. Remaining work is longer robustness testing across block/review/prompt-injection cases.
...
**Acceptance evidence:** `tests/test_auditor.py` proves stray `YES` in reasoning does not approve; malformed output blocks; structured allow/block/review outputs parse correctly. `venv/bin/python scripts/auditor_smoke.py` exits 0 in a Metal-capable session with a structured real-model verdict. Broader real-model schema adherence still needs more adversarial smoke cases.
