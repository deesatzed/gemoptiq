...
def test_readiness_report_can_run_named_codex_tui_smoke(tmp_path):
    commands = []

    def fake_runner(command, cwd, project_root):
        commands.append(command)
        if "real_agent_smoke.py" in command and "--codex-tui-smoke" in command:
            return {
                "returncode": 0,
                "output_tail": json.dumps(
                    {
                        ...
                        "interactive_prompt_control": True,
                        "proves_model_or_tool_behavior": True,
                    }
                ),
            }
    ...
    report = build_report(
        ...
        run_full_tests=False,
        run_real_auditor=False,
        run_codex_tui_smoke=True,
        runner=fake_runner,
    )

    assert report["items"]["real_agent_codex_tui_smoke"]["status"] == "pass"
    assert any("--codex-tui-smoke" in command for command in commands)
